...
include("network_STG_gs_derivatives.jl") # Include derivatives of model gating functions (dX/dV)
...
# This function computes the values of (gCaS, gA) that generate the values of the DICs (slow and uslow)
# at threshold voltage for the given set of conductances (gNa, gCaT, gKd, gKCa, gH, gleak)
function DICs_gmax_neuromodCaSA_faster(gNa, gCaT, gKd, gKCa, gH, gleak, gs_th, gu_th, Vth, tmKCa)
  # Defining the 3 time constants of the 3 timescales
  tau_fast = tau_mNa(Vth)
  tau_slow = tau_mKd(Vth)
  tau_uslow = tau_mH(Vth)
  ...
  # Solving the linear system
  g_sol = zeros(2)
  g_sol[2] = (B[1, 1] - (A[1, 1]/A[2, 1])*B[2, 1]) / (A[1, 2] - (A[1, 1]/A[2, 1])*A[2, 2])
  g_sol[1] = (B[2, 1] - A[2, 2]*g_sol[2]) / A[2, 1]
  return (g_sol[1], g_sol[2])
end

function compute_sensitivity(Vth, tmKCa)
    # Defining the 3 time constants of the 3 timescales
    tau_fast = tau_mNa(Vth)
    tau_slow = tau_mKd(Vth)
    tau_uslow = tau_mH(Vth)
    ...
    # Filling the sensitivity matrix
    S = zeros(2, 7)
    S[1, 1] = (wsu_mNa - wfs_mNa)*3*mNa_inf(Vth)^2*hNa_inf(Vth)*(Vth-VNa)*dmNa(Vth) + (wsu_hNa - wfs_hNa)*mNa_inf(Vth)^3*(Vth-VNa)*dhNa(Vth)
    S[1, 2] = (wsu_mCaT - wfs_mCaT)*3*mCaT_inf(Vth)^2*hCaT_inf(Vth)*(Vth-VCa)*dmCaT(Vth) + (wsu_hCaT- wfs_hCaT)*mCaT_inf(Vth)^3*(Vth-VCa)*dhCaT(Vth)
    S[1, 3] = (wsu_mCaS - wfs_mCaS)*3*mCaS_inf(Vth)^2*hCaS_inf(Vth)*(Vth-VCa)*dmCaS(Vth) + (wsu_hCaS - wfs_hCaS)*mCaS_inf(Vth)^3*(Vth-VCa)*dhCaS(Vth)
    S[1, 4] = (wsu_mA - wfs_mA)*3*mA_inf(Vth)^2*hA_inf(Vth)*(Vth-VK)*dmA(Vth) + (wsu_hA - wfs_hA)*mA_inf(Vth)^3*(Vth-VK)*dhA(Vth)
    S[1, 6] = (wsu_mKd - wfs_mKd)*4*mKd_inf(Vth)^3*(Vth-VK)*dmKd(Vth)
    S[1, 7] = (wsu_mH - wfs_mH)*(Vth-VH)*dmH(Vth)
    S[2, 1] = (1 - wsu_mNa)*3*mNa_inf(Vth)^2*hNa_inf(Vth)*(Vth-VNa)*dmNa(Vth) + (1 - wsu_hNa)*mNa_inf(Vth)^3*(Vth-VNa)*dhNa(Vth)
    S[2, 2] = (1 - wsu_mCaT)*3*mCaT_inf(Vth)^2*hCaT_inf(Vth)*(Vth-VCa)*dmCaT(Vth) + (1- wsu_hCaT)*mCaT_inf(Vth)^3*(Vth-VCa)*dhCaT(Vth)
    S[2, 3] = (1 - wsu_mCaS)*3*mCaS_inf(Vth)^2*hCaS_inf(Vth)*(Vth-VCa)*dmCaS(Vth) + (1 - wsu_hCaS)*mCaS_inf(Vth)^3*(Vth-VCa)*dhCaS(Vth)
    S[2, 4] = (1 - wsu_mA)*3*mA_inf(Vth)^2*hA_inf(Vth)*(Vth-VK)*dmA(Vth) + (1 - wsu_hA)*mA_inf(Vth)^3*(Vth-VK)*dhA(Vth)
    S[2, 6] = (1 - wsu_mKd)*4*mKd_inf(Vth)^3*(Vth-VK)*dmKd(Vth)
    S[2, 7] = (1 - wsu_mH)*(Vth-VH)*dmH(Vth)
...
# This function computes the values of (gCaS, gA) that generate the values of the DICs (slow and uslow)
# at threshold voltage for the given set of conductances (gNa, gCaT, gKd, gKCa, gH, gleak)
function DICs_gmax_neuromodCaSA_fastest(S, gNa, gCaT, gKd, gKCa, gH, gleak, gs_th, gu_th, Vth, tmKCa)
  # Defining the 3 time constants of the 3 timescales
  tau_fast = tau_mNa(Vth)
  tau_slow = tau_mKd(Vth)
  tau_uslow = tau_mH(Vth)

  # Computing the wfs(V) and wsu(V) for KCa gating variables
  (wfs_mKCa, wsu_mKCa) = var_contribution(tau_mKCa(Vth, tmKCa), tau_fast, tau_slow, tau_uslow)
  ...
  # Filling the B matrix of the compensation algorithm (= contribution of the non-unknowns to the DICs)
  B[1, 1] = gs_th - (1/gleak) * (S[1, 1]*gNa + S[1, 2]*gCaT + (wsu_mKCa - wfs_mKCa)*gKCa*4*mKCa_inf(Vth, Ca_th)^3*(Vth-VK)*dmKCa(Vth, Ca_th) +
                                (wsu_mKCa2 - wfs_mKCa2)*gKCa*4*mKCa_inf(Vth, Ca_th)^3*(Vth-VK).*dmKCa2(Vth, Ca_th).*dCa(Vth, Ca_th, gCaT, 10.) +
                                S[1, 6]*gKd + S[1, 7]*gH)
  B[2, 1] = gu_th - (1/gleak) * (S[2, 1]*gNa + S[2, 2]*gCaT + (1 - wsu_mKCa)*gKCa*4*mKCa_inf(Vth, Ca_th)^3*(Vth-VK)*dmKCa(Vth, Ca_th) +
