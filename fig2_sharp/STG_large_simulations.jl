...
function simulate_STG_population(g_all_init, Iapp, tau_Na, tau_g, Ca_tgt,
    ...
    @showprogress "Computing..." for i = 1 : ncells
        ...
        gleak_matrix[i, Int(round(end/2))+1:end] = x2[21, :]
    end
...
## STG model from Liu 1998 - current-clamp mode
function simulate_STG_population_washout(g_all_init, gNa_matrix_init, gCaT_matrix_init, gCaS_matrix_init, gA_matrix_init, gKCa_matrix_init, gKd_matrix_init, gH_matrix_init, 
gleak_matrix_init, Ca_ma_matrix_init, Iapp, tau_Na, tau_g, Ca_tgt, C, ICs_th_init, tt, tt_g, i_init)

    # Initializing some variables
    gNa_matrix = zeros(ncells, length(tt_g))
    gNa_matrix[:, 1:i_init] = deepcopy(gNa_matrix_init[:, 1:i_init])
    gCaT_matrix = zero(gNa_matrix)
    gCaT_matrix[:, 1:i_init] = deepcopy(gCaT_matrix_init[:, 1:i_init])
    gCaS_matrix = zero(gNa_matrix)
    gCaS_matrix[:, 1:i_init] = deepcopy(gCaS_matrix_init[:, 1:i_init])
    gA_matrix = zero(gNa_matrix)
    gA_matrix[:, 1:i_init] = deepcopy(gA_matrix_init[:, 1:i_init])
    gKCa_matrix = zero(gNa_matrix)
    gKCa_matrix[:, 1:i_init] = deepcopy(gKCa_matrix_init[:, 1:i_init])
    gKd_matrix = zero(gNa_matrix)
    gKd_matrix[:, 1:i_init] = deepcopy(gKd_matrix_init[:, 1:i_init])
    gH_matrix = zero(gNa_matrix)
    gH_matrix[:, 1:i_init] = deepcopy(gH_matrix_init[:, 1:i_init])
    gleak_matrix = zero(gNa_matrix)
    gleak_matrix[:, 1:i_init] = deepcopy(gleak_matrix_init[:, 1:i_init])
    ...
    if i_init == 161
        Ca_ma_matrix[:, 1:Int(round(1452*161/300))] = deepcopy(Ca_ma_matrix_init[:, 1:Int(round(1452*161/300))])
    else
        Ca_ma_matrix[:, 1:1452] = deepcopy(Ca_ma_matrix_init)
    end

    @showprogress "Computing..." for i = 1 : ncells
        # Extracting the maximal ion channel conductances
        gNa = gNa_matrix[i, i_init]
        gCaT = gCaT_matrix[i, i_init]
        gCaS = gCaS_matrix[i, i_init]
        gA = gA_matrix[i, i_init]
        gKCa = gKCa_matrix[i, i_init]
        gKd = gKd_matrix[i, i_init]
        gH = gH_matrix[i, i_init]
        gleak = gleak_matrix[i, i_init]
        g_all_simu = (gNa, gCaT, gCaS, gA, gKCa, gKd, gH, gleak)

        # Computing homeostasis time constants
        (gNa_tau, gCaT_tau, gCaS_tau, gA_tau, gKCa_tau, gKd_tau, gH_tau, gleak_tau) = g_all_init[i, :]
        tau_CaT = tau_Na * gNa_tau / gCaT_tau
        tau_CaS = tau_Na * gNa_tau / gCaS_tau
        tau_A = tau_Na * gNa_tau / gA_tau
        tau_KCa = tau_Na * gNa_tau / gKCa_tau
        tau_Kd = tau_Na * gNa_tau / gKd_tau
        tau_H = tau_Na * gNa_tau / gH_tau
        tau_leak = tau_Na * gNa_tau / gleak_tau

        # Applying washout
        gCaS_washout = gCaS_matrix_init[i, 150]
        gA_washout = gA_matrix_init[i, 150]
        ...
        x0  = [V0, mNa_inf(V0), hNa_inf(V0), mCaT_inf(V0), hCaT_inf(V0), mCaS_inf(V0), hCaS_inf(V0),
                mA_inf(V0), hA_inf(V0), mKCa_inf(V0, Ca0), mKd_inf(V0), mH_inf(V0), Ca0,
                gNa, gCaT, gCaS_washout, gA_washout, gKCa, gKd, gH, gleak, gNa, gCaT, gCaS_washout, gA_washout, gKCa, gKd, gH, gleak]

        # Simulation
        prob = ODEProblem(STG_homeo_leak_ODE, x0, (tt_g[i_init]*1e3, Tfinal), p) # Describing the problem
        sol = solve(prob) # Solving the problem

        # Interpolating solution
        i_tt = findall(abs.(tt./1e3 .- tt_g[i_init]) .< 0.00005 )[1]

        # Retrieving variables
        x = sol(tt[i_tt:5000:end])

        # Pushing variables into matrices
        if i_init == 161
            Ca_ma_matrix[i, Int(round(1452*161/300))+1:end] = moving_average(sol(tt)[13, Int(round(end*160.5/450)):end], window_size_, padding_)
        else
            temp = moving_average(sol(tt)[13, Int(round(end*300/450)):end], window_size_, padding_)
            Ca_ma_matrix[i, 1453:end] = linear_interpolation(range(0,1,length(temp)), temp).(range(0,1,750))
        end
        gNa_matrix[i, i_init:end]   = x[14, :]
        gCaT_matrix[i, i_init:end]  = x[15, :]
        gCaS_matrix[i, i_init:end]  = x[16, :]
        gA_matrix[i, i_init:end]    = x[17, :]
        gKCa_matrix[i, i_init:end]  = x[18, :]
        gKd_matrix[i, i_init:end]   = x[19, :]
        gH_matrix[i, i_init:end]    = x[20, :]
        gleak_matrix[i, i_init:end] = x[21, :]
    end
...
## STG model from Liu 1998 - current-clamp mode
function simulate_STG_first_half_H(g_all, Iapp, tau_Na, tau_CaT, tau_CaS, tau_A, tau_KCa, tau_Kd, tau_H, tau_leak, Ca_tgt, C)
    ## Simulation of the model in current-clamp mode
    # Extracting the maximal ion channel conductances
    (gNa, gCaT, gCaS, gA, gKCa, gKd, gH, gleak) = g_all
    ...
    # Simulation
    prob = ODEProblem(STG_homeo_leak_ODE_H, x0, (0, Tfinal/2), p) # Describing the problem
    sol = solve(prob) # Solving the problem
...
## STG model from Liu 1998 - current-clamp mode
function simulate_STG_second_half_H(g_all, gCaS_init_crash, gH_init_crash, Iapp, tau_Na, tau_CaT, tau_CaS, tau_A, tau_KCa, tau_Kd, tau_H, tau_leak, Ca_tgt, C)
    ## Simulation of the model in current-clamp mode
    # Extracting the maximal ion channel conductances
    (gNa, gCaT, gCaS, gA, gKCa, gKd, gH, gleak) = g_all
    ...
    p = (Iapp, tau_Na, tau_CaT, tau_CaS, tau_A, tau_KCa, tau_Kd, tau_H, tau_leak, tau_g, Ca_tgt, C)
     ...
     x0  = [V0, mNa_inf(V0), hNa_inf(V0), mCaT_inf(V0), hCaT_inf(V0), mCaS_inf(V0), hCaS_inf(V0),
            mA_inf(V0), hA_inf(V0), mKCa_inf(V0, Ca0), mKd_inf(V0), mH_inf(V0), Ca0,
            gNa, gCaT, gCaS_init_crash, gA, gKCa, gKd, gH_init_crash, gleak, gNa, gCaT, gCaS_init_crash, gA, gKCa, gKd, gH_init_crash, gleak]
...
## STG model from Liu 1998 - current-clamp mode
function simulate_STG_population_beautiful_H(g_all_init, Iapp, tau_Na, tau_g, Ca_tgt,
                                           C, ICs_th_init, tt)
    # Initializing some variables
    gNa_matrix = zeros(ncells, length(tt[2:5000:end]))
    gCaT_matrix = zero(gNa_matrix)
    ...
    @showprogress "Computing..." for i = 1 : ncells
        ...
        # Simulate to initial blockade conditions
        sol1 = simulate_STG_first_half_H(g_all_init[i, :], Iapp, tau_Na, tau_CaT, tau_CaS, tau_A, tau_KCa, tau_Kd, tau_H, tau_leak, Ca_tgt, C)
        ...
        (gCaS_init_crash, gH_init_crash) = DICs_gmax_neuromodCaSH(gNa_matrix[i, j_beautiful], gCaT_matrix[i, j_beautiful],
            gA_matrix[i, j_beautiful], gKd_matrix[i, j_beautiful], gKCa_matrix[i, j_beautiful], gleak_matrix[i, j_beautiful], -8., 4., ICs_th_init[i, 1])

        # Simulate to initial blockade conditions
        g_all_init2 = [gNa_matrix[i, j_beautiful], gCaT_matrix[i, j_beautiful], gCaS_matrix[i, j_beautiful], gA_matrix[i, j_beautiful],
                       gKCa_matrix[i, j_beautiful], gKd_matrix[i, j_beautiful], gH_matrix[i, j_beautiful], gleak_matrix[i, j_beautiful]]
        sol2 = simulate_STG_second_half_H(g_all_init2, gCaS_init_crash, gH_init_crash, Iapp,
                                         tau_Na, tau_CaT, tau_CaS, tau_A, tau_KCa, tau_Kd, tau_H, tau_leak, Ca_tgt, C)
