...
function DA_ODE(du, u, p, t)
    ...
    mNa    = u[2]  # Activation gating variable of current Na
	hNa    = u[3]  # Inactivation gating variable of current Na
	mKd    = u[4]  # Activation gating variable of current Kd
	mCaL   = u[5]  # Activation gating variable of current CaL
	mCaN   = u[6]  # Activation gating variable of current CaN
	oERG   = u[7]  # ERG potassium current activation
	iERG   = u[8]  # ERG current intermediate
	Ca     = u[9]  # Intracellular calcium

    # ODEs
	du[1] = (1/C) * (- gNa*mNa^3*hNa^1*(V - 60.0) -
					   gKd*mKd^3*(V - -85.0) -
					   gCaL*mCaL^2*(V - 60.0) -
					   gCaN*mCaN^1*(V - 60.0) -
					   gERG*oERG^1*(V - -85.0) -
					   gNMDA*DA_NMDA_inf(V, Mg)^1*(V - 0.0) -
					   gleak*(V - -50.0) + Iapp)
	du[2] = (1/DA_tau_mNa(V)) * (DA_mNa_inf(V) - mNa)
	du[3] = (1/DA_tau_hNa(V)) * (DA_hNa_inf(V) - hNa)
	du[4] = (1/DA_tau_mKd(V)) * (DA_mKd_inf(V) - mKd)
	du[5] = (1/DA_tau_mCaL(V)) * (DA_mCaL_inf(V) - mCaL)
	du[6] = (1/DA_tau_mCaN(V)) * (DA_mCaN_inf(V) - mCaN)
	du[7] = a0ERG(V) * (1-oERG-iERG) + biERG(V) * iERG - oERG * (aiERG(V)+b0ERG(V))
    du[8] = aiERG(V) * oERG - biERG(V) * iERG

	# Added calcium dynamics
	du[9] = (-0.94*gCaL*mCaL^2*(V - 60.0) + -0.94*gCaN*mCaN^1*(V - 60.0) + -Ca + 0.05) / 20.0
end