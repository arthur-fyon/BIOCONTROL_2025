...
## STG model from Liu 1998 - current-clamp mode
function simulate_STG_H(g_all, Iapp, tau_Na, tau_g, Ca_tgt, C, α, β, Kp, Ki, Kt,
                      gsth_sim, guth_sim, Vth, u_maxCaS, u_maxH)
    ## Simulation of the model in current-clamp mode
    # Extracting the maximal ion channel conductances
    (gNa, gCaT, gCaS, gA, gKCa, gKd, gH, gleak) = g_all
    ...
    # Parameter vector for simulations
    p = (Iapp, tau_Na, tau_CaT, tau_A, tau_KCa, tau_Kd, tau_leak, tau_g, Ca_tgt, C, α, β,
         Kp, Ki, Kt, gsth_sim, guth_sim, Vth, u_maxCaS, u_maxH)
     ...
     x0  = [V0, mNa_inf(V0), hNa_inf(V0), mCaT_inf(V0), hCaT_inf(V0), mCaS_inf(V0),
            hCaS_inf(V0), mA_inf(V0), hA_inf(V0), mKCa_inf(V0, Ca0), mKd_inf(V0), mH_inf(V0),
            Ca0, gNa, gCaT, gA, gKCa, gKd, gleak, gNa, gCaT, gA, gKCa, gKd, gleak,
            gCaS, gCaS, (β * gCaS) / Ki, gH, gH, (β * gH) / Ki]

    # Simulation
    prob = ODEProblem(simple_PI_H_homeo_leak_STG_ODE, x0, tspan, p) # Describing the problem
    sol = solve(prob) # Solving the problem
...
## STG model from Liu 1998 - current-clamp mode
function simulate_STG_population_H(g_all_init, Iapp, tau_Na, tau_g, Ca_tgt,
                                 C, α, β, Kp, Ki, Kt, gsth_sim, guth_sim,
    ...
    @showprogress "Computing..." for i = 1 : ncells
        # Simulate to initial blockade conditions
        sol = simulate_STG_H(g_all_init[i, :], Iapp, tau_Na, tau_g, Ca_tgt, C, α, β, Kp, Ki, Kt,
                           gsth_sim, guth_sim, ICs_th_init[:, 1][i], u_maxCaS, u_maxH)
